...
**Pipeline скрейпинга:**
```
Cron Job (почасово) → Scraper Service → Playwright (headless браузер) → 
Parse HTML (селекторы Cheerio) → Перевод & Нормализация → 
...
| **Playwright** | Headless браузер | 1.58.2 | Ради больше контроля |
| **Cheerio** | HTML парсинг | 1.2.0 | Требвоание из ТЗ |
...
| **Tailwind CSS** | Стилизация | 4.0 | Выбран для быстрое создания стилей\дизайна компонентов |
